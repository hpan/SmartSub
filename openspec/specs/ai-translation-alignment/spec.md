...
AI 批量翻译的条数与语义对齐保证：按批次动态生成 JSON Schema 在解码层面锁死键集合；回显锚定（模型逐条返回 {src: 原文回显, tr: 译文}）确定性检测合并/滑移错位；失败分级（整批重试一次 → 单条定点补翻 → 单条标记失败）确保错位不入库、单条失败不污染整批。源自 2026-07 调研与实测（详见 archive/2026-07-17-ai-translation-alignment）。
